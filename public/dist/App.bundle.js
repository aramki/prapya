...
var dompurity = __webpack_require__(9);
...
function loadAround(map) {
    var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 12.96;
    var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 77.72;

    axios.get('/apiv1/stores/around?lat=' + lat + '&lng=' + lng).then(function (res) {
        var places = res.data;
        if (!places.length) {
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        axios.get('/apiv1/search?q=' + this.value).then(function (res) {
            if (res.data.length) {
                searchResults.innerHTML = dompurity.sanitize(searchResultsHTML(res.data));
                return;
            }
            searchResults.innerHTML = dompurity.sanitize('<div class="search__result">No Results found for ' + _this.value + '</div>');
        }).catch(function (err) {
            console.log(err);
        });
    });
